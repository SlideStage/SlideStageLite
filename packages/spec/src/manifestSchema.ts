...
import {
  ...
  SCHEMA_LITERAL,
  SIZE_LIMITS,
  SUPPORTED_PLATFORM_SCHEMA_VERSION,
  TRUST_CAPABILITIES,
  type TrustCapability,
} from './constants';
import { DeckLoadError, type Manifest } from './types';

const architectureSchema = z.enum(ARCHITECTURES);

const knownTrustCapabilities = new Set<TrustCapability>(TRUST_CAPABILITIES);
...
export const manifestSchema = z
  .object({
    ...
    totalSlides: z.number().int().positive().max(SIZE_LIMITS.totalSlidesMax),
    slides: z
      .array(
        z
          .object({
      ...
      .min(1)
      .max(SIZE_LIMITS.totalSlidesMax),
...
/**
 * Default `onWarning` handler that mirrors the legacy `@slidestage/core`
 * `parseManifest` side-effect: every {@link ManifestWarning} is written
 * to `console.warn` when a global `console` is available.
 *
 * The spec itself never touches `console` so that it stays usable from
 * any runtime (browser, Worker, Node, Deno, Bun, even environments that
 * strip `console`). Consumers that want the old behavior — most notably
 * `@slidestage/core`'s `parseManifest` re-export — wire this in as the
 * default callback:
 *
 * ```ts
 * import { parseManifest as parseManifestRaw, logManifestWarningToConsole } from '@slidestage/spec';
 *
 * export function parseManifest(value: unknown, options: ParseManifestOptions = {}) {
 *   return parseManifestRaw(value, {
 *     onWarning: options.onWarning ?? logManifestWarningToConsole,
 *   });
 * }
 * ```
 */
export function logManifestWarningToConsole(warning: ManifestWarning): void {
  const consoleRef = (globalThis as { console?: { warn?: (...args: unknown[]) => void } })
    .console;
  const warn = consoleRef?.warn;
  if (!warn) return;
  switch (warning.code) {
    case 'totalSlidesMismatch':
      warn(
        `[slidestage] manifest.totalSlides (${warning.declared}) does not match slides.length (${warning.actual}); using slides.length.`,
      );
      return;
    case 'slideIndexRenumbered':
      warn(
        `[slidestage] slides[].index was not sequential (${warning.originalIndices.join(', ')}); renumbering by array order.`,
      );
      return;
    case 'unknownCompatCapability':
      warn(
        `[slidestage] manifest.compat.requires contains unknown capability "${warning.capability}"; ignoring it.`,
      );
      return;
  }
}
...
function validatePlatformVersion(manifest: Manifest): void {
  ...
  if (compareSemver(minVersion, SUPPORTED_PLATFORM_SCHEMA_VERSION) > 0) {
    throw new DeckLoadError(
      'E_UNSUPPORTED_SCHEMA',
      `manifest requires platform schema >= ${minVersion}, but spec supports ${SUPPORTED_PLATFORM_SCHEMA_VERSION}.`,
    );
  }
}
